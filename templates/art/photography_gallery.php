...
<div class="min-h-screen flex flex-col items-center bg-neutral-50">
    ...
    <div class="max-w-6xl px-4 py-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <?php foreach ($photos as $index => $photo): ?>
            <a href="#photo-<?= $index ?>" class="block">
                <img src="/assets/images/<?= $directory ?>/<?= basename($photo) ?>"
                     alt="Photo <?= $index + 1 ?>"
    ...
    <div class="max-w-6xl space-y-16 px-4 py-4">
        <?php foreach ($photos as $index => $photo): ?>
            <img id="photo-<?= $index ?>"
                 src="/assets/images/<?= $directory ?>/<?= basename($photo) ?>"
                 alt="Photo <?= $index + 1 ?>"
...
<script>
    ...
    function openFullscreen(index) {
        ...
        img.src = `/assets/images/${dir}/${basename(images[index])}`;
    ...
    function showImage(index) {
        if(index >= 0 && index < images.length) {
            currentIndex = index;
            document.getElementById('fullscreenImage').src = `/assets/images/${dir}/${basename(images[currentIndex])}`;
        }
    }
